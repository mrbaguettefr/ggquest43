...
* The editable Tiled exploration map is `public/assets/Exploration/tileset/map.tmj`, with external tilesets stored as `.tsj` files in the same folder. The current map uses the `walls`, `stone-ground`, `props`, `skeleton`, and `grass-ground` tilesets, and its tile layers are named `p-ground-1`, `ground-1`, `walls-1`, `ground-1-deco`, `ground-1-deco-blocking`, and `ground-1-deco-2`. Player movement is allowed on any non-empty `ground-1` tile except positions with a tile on `ground-1-deco-blocking`; `walls-1` is visual and does not block movement by itself. The Vite `tiledMapPlugin` inlines those tilesets into `public/assets/Exploration/tileset/map-tiled.json` before dev and production builds; Phaser loads the generated `map-tiled.json`.
