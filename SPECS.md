...
* `src/game/scenes/Battle.ts` owns fullscreen combat rendering, target selection, turn resolution, victory, defeat, and returning battle results to Exploration. Won battles restart Exploration at the saved pre-battle map position; lost battles resurrect Cloud at the Card Reader wall.
  * `src/game/scenes/GameOver.ts` remains available as a standalone Game Over scene

* Cloud's exploration sprite uses the `public/assets/characters/heroes/cloud/exploration/cloud2-iso_{idle,run}_{down,right,up}-v1.*` image and atlas JSON files. The custom atlas JSON is adapted into Phaser atlas textures during Preloader setup, then Exploration uses directional idle animations while standing and directional run animations while moving. Left-facing movement reuses the right-facing run/idle atlases and flips the player sprite horizontally with Phaser's flip API.
* After King Slime is defeated, Leon unlocks and appears at the exploration map object named `leon` until recruited. Exploration renders Leon with `public/assets/characters/heroes/leon/exploration/leon3-iso_idle_down-v1.*`, plays the idle-down animation, and lets the player press E nearby to recruit him. After the blue card unlock, Mistress appears at the exploration map object named `mistress`; Exploration renders her with `public/assets/characters/heroes/mistress/exploration/mistress2-iso_idle_down-v1.*`, plays the idle-down animation, and lets the player press E nearby to recruit her.
* The `baguettefr` NPC is placed from the exploration map object named `baguettefr`. Exploration renders the NPC with `public/assets/characters/npcs/baguettefr/exploration/baguettefr-iso_idle_down-v1.*`, plays the idle-down animation, and lets the player press E nearby to open a dialog message. The NPC dialog uses the same centered Exploration message format as hero recruitment and changes hints based on Card Reader progress.
* Battle renders Cloud with `public/assets/characters/heroes/cloud/battle/cloud2-idle-v2.*`, recruited Leon with `public/assets/characters/heroes/leon/battle/leon3-idle-v1.*`, and recruited Mistress with `public/assets/characters/heroes/mistress/battle/mistress2-idle-v1.*`. Their matching `*-attack-v1.*` atlases play when the hero attacks, then the sprite returns to idle. Generic enemies without dedicated battle art still use the old `public/assets/characters/heroes/cloud/battle/cloud-idle-v1.*` placeholder with a tint. Monsters and bosses that have AutoSprite side-scrolling idle art use one shared idle atlas key for both exploration map idle and battle idle, with no separate battle-only idle spritesheet. Wolf Rider uses the neutral atlas key `wolf-rider2-idle`, loaded from `public/assets/characters/monsters/wolf-rider2/exploration/wolf-rider2-iso_idle_down-v1.*`, for both map idle and battle idle, plus its battle attack and death atlases. When a matching JSON file exists beside a battle character, monster, or boss PNG, Preloader adapts that JSON into a Phaser atlas texture instead of slicing frames by guessed dimensions. If a battle attack animation is missing, Battle falls back to the unit idle animation; if battle art is missing, the encounter can use the exploration idle atlas instead. Cloud's battle sprite is flipped horizontally so he faces the enemies. Target selection uses the cropped image asset `public/assets/battle/finger.png`.
* Battle background art is selected from the current exploration area: Area 1 / The Verdant Plains uses `public/assets/battle/areas/plains/graphics/battlefield-plains.png`, Area 2 / The Hollow Dungeon uses `public/assets/battle/areas/dungeon/graphics/battlefield-dungeon.png`, and Area 3 / The Molten Underdeep uses `public/assets/battle/areas/lava/graphics/battlefield-lava-underground.png`.
...
* Exploration map objects named `enemy` can define battlefield stacks with Tiled custom properties in the format `<position>_<enemy_key> = <count>`, such as `1_slime = 4`. Positions 1 through 5 map from top to bottom on the enemy side of the battlefield, and each property becomes one enemy group in battle. Supported enemy keys include `skeleton`, `skeleton_archer`, `slime`, `wolf_rider`, `magma_golem`, and `revenant`; missing art falls back to the generic battle placeholder or exploration idle art. Each enemy group attacks once during the enemy phase, even when the group count is greater than 1.
* Exploration map objects named `boss` with a `king-slime` custom property start the King Slime boss encounter when the player stands nearby and presses E. Boss objects can also use boss custom properties such as `bile-demon` or `cyberdemon` to create a single boss group in the middle enemy position. King Slime uses `public/assets/characters/bosses/king-slime/battle/king-slime-boss-idle-v1.*` for both map idle and battle idle, has high HP with weak attack damage, awards the Blue Card, and unlocks Leon when defeated.
* Runtime assets are grouped by domain. Character sprites and atlases are grouped by role and identity under `public/assets/characters/{heroes,npcs,monsters,bosses}/<name>/{exploration,battle}/`. Non-character assets live under feature folders: `main-menu`, `wall`, `exploration`, `battle`, and `shared`. The early boot background is `public/assets/shared/ui/bg.png`. Source/reference files from the tile pack are kept outside the served runtime assets under `asset_sources/top_down_basic_v1.2.3/`.
* Music is managed through `src/game/music.ts` and loaded by Preloader from domain folders. Exploration crossfades between `public/assets/exploration/audio/center.m4a`, `public/assets/exploration/areas/plains/audio/plains.m4a`, `public/assets/exploration/areas/dungeon/audio/dungeon.m4a`, and `public/assets/exploration/areas/lava/audio/lava.m4a` based on whether Cloud is in the center hub, The Verdant Plains, The Hollow Dungeon, or The Molten Underdeep. Battle randomly starts one of the shared battle tracks `public/assets/battle/audio/battle1.m4a`, `battle2.m4a`, or `battle3.m4a` on entry, using a faster battle crossfade than area-to-area exploration music changes.
* The editable Tiled exploration map is `public/assets/exploration/tilemaps/map.tmj`, with external tilesets stored as `.tsj` files under `public/assets/exploration/tilesets/` and runtime tileset PNGs under `public/assets/exploration/tilesets/textures/`. The current map uses the `walls`, `stone-ground`, `props`, `skeleton`, `grass-ground`, `TX Plant`, `lava-ground`, `lava-walls`, and `structs` tilesets, and its tile layers render in this order: `p-ground-1`, `ground-1`, `ground-1-deco`, `walls-1`, `ground-1-deco-blocking`, and `ground-1-deco-2`. Player movement is allowed on any non-empty `ground-1` tile except positions with a tile on `ground-1-deco-blocking`; `walls-1` is visual and does not block movement by itself. Exploration reads gameplay objects from the `bg-objects` and `objects` object layers, including area polygons, start position, wall interaction point, enemies, NPCs, recruitable heroes, and tile objects named `plant`. Plant tile objects are drawn from the `TX Plant` tileset at their Tiled object positions. The Vite `tiledMapPlugin` inlines those tilesets into `public/assets/exploration/tilemaps/map-tiled.json` before dev and production builds; Phaser loads the generated `map-tiled.json`.
* `src/game/gameSession.ts` creates the shared game session object passed between Seed, PlayerName, Exploration, and Battle
* `src/game/main.ts` creates the Phaser `Game` with an `AUTO` renderer type, allowing Phaser to choose the available runtime renderer instead of forcing canvas-only mode
...
* `scripts/crop-png.mjs` trims PNG image borders made of transparent pixels, white pixels, or near-white pixels. Run it with `node scripts/crop-png.mjs <input.png>`.
* The default output is written next to the source image with a `-cropped.png` suffix, so `sprite.png` becomes `sprite-cropped.png` without overwriting the source.
* The script supports `--tolerance <0-255>` to control how close to white a border pixel can be and `--output <path>` to choose a custom output path. `--flood-background` makes the connected background from the image edges transparent before cropping, with `--flood-threshold <0-255>` controlling the neighbor RGB distance. If the image is entirely transparent or white within tolerance, the script fails with a clear error instead of writing an empty image.
...
* Enemies occupy up to five named positions on the opposite side; player heroes occupy up to three positions on the left
* Simple idle/attack animations are used when available, with idle/exploration fallbacks when an animation or battle sheet is missing
...
4. Each living hero resolves their attack one by one, playing an attack animation when available

5. Enemy turn begins

6. Each living enemy group attacks automatically one by one
...
* Enter battle: quick fade to combat with a fast combat music crossfade
