...
* The script supports `--tolerance <0-255>` to control how close to white a border pixel can be and `--output <path>` to choose a custom output path. If the image is entirely transparent or white within tolerance, the script fails with a clear error instead of writing an empty image.
