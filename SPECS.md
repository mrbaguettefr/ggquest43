...
* Battle renders Cloud with `public/assets/Battle/characters/cloud2-idle-v2.*`, recruited Leon with `public/assets/Battle/characters/leon3-idle-v1.*`, and recruited Mistress with `public/assets/Battle/characters/mistress2-idle-v1.*`. Their matching `*-attack-v1.*` atlases play when the hero attacks, then the sprite returns to idle. Generic enemies without dedicated battle art still use the old `public/assets/Battle/characters/cloud-idle-v1.*` placeholder with a tint. When a matching JSON file exists beside a battle character or monster PNG, Preloader adapts that JSON into a Phaser atlas texture instead of slicing frames by guessed dimensions. If a battle attack animation is missing, Battle falls back to the unit idle animation; if battle art is missing, the encounter can use the exploration idle atlas instead. Cloud's battle sprite is flipped horizontally so he faces the enemies.
...
* Exploration map objects named `boss` with a `king-slime` custom property start the King Slime boss encounter when the player stands nearby and presses E. Boss objects can also use boss custom properties such as `bile-demon` or `cyberdemon` to create a single boss group in the middle enemy position. King Slime uses `public/assets/Exploration/world/monsters/king-slime-boss-iso_idle_right-v1.*` on the map, uses `public/assets/Battle/monsters/king-slime-boss-idle-v1.*` in battle, has high HP with weak attack damage, awards the Blue Card, and unlocks Leon when defeated.
* Scene-owned assets are grouped under `public/assets/<SceneName>/` when they are only used by that scene. `MainMenu` owns its logo, `Exploration` owns world characters, monsters, and tilemap assets, `Wall` owns the Card Reader wall background, and `Battle` owns combat background and battle character sprites. The shared assets root is reserved for runtime assets used across scenes; currently `public/assets/bg.png` and global music files live there.
* Music is managed through `src/game/music.ts` and loaded by Preloader from shared root assets. Exploration crossfades between `center.m4a`, `plains.m4a`, `dungeon.m4a`, and `lava.m4a` based on whether Cloud is in the center hub, The Verdant Plains, The Hollow Dungeon, or The Molten Underdeep. Battle randomly starts one of `combat1.m4a`, `combat2.m4a`, or `combat3.m4a` on entry, using a faster combat crossfade than area-to-area exploration music changes.
