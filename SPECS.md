...
* When Cloud is within interaction range, Exploration shows the matching `Press E` prompt and draws a white rounded outline around the current target. Sprite-based interactables use their world sprite bounds, while point-based interactions such as the Card Reader wall and the knight recruit spot use a fixed world-space outline box.
